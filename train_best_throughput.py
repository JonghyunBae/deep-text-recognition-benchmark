...
def train(opt):
    ...
    s = torch.cuda.Stream(priority=high)
...
#        torch.nn.utils.clip_grad_norm_(model.parameters(), opt.grad_clip)  # gradient clipping with 5 (Default)
        ...
        if i == opt.num_iter:
            with open(f'./saved_models/{opt.experiment_name}/throughput.txt', 'a') as log:
                avg_throughput = avg_throughput / (opt.num_iter / opt.printInterval)
                throughput_log = f'end the training, average throughput: {avg_throughput:0.5f}'
                print(throughput_log)
                log.write(throughput_log + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eps', type=float, default=1e-4, help='eps for Adadelta. default=1e-8')
