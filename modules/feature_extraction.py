...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        conv1 = self._conv3x3(inplanes, planes)
        bn1 = nn.BatchNorm2d(planes)
        self.fusedconv1 = fused_conv_bn(conv1, bn1)
        conv2 = self._conv3x3(planes, planes)
        bn2 = nn.BatchNorm2d(planes)
        self.fusedconv2 = fused_conv_bn(conv2, bn2)
    ...
    def forward(self, x):
        ...
        out = self.fusedconv1(x)
        out = self.relu(out)

        out = self.fusedconv2(out.float())
...
class ResNet(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, block, layers):
        ...
        conv0_1 = nn.Conv2d(input_channel, int(output_channel / 16),
                                 kernel_size=3, stride=1, padding=1, bias=False)
        bn0_1 = nn.BatchNorm2d(int(output_channel / 16))
        self.fusedconv0_1 = fused_conv_bn(conv0_1, bn0_1)

        conv0_2 = nn.Conv2d(int(output_channel / 16), self.inplanes,
                                 kernel_size=3, stride=1, padding=1, bias=False)
        bn0_2 = nn.BatchNorm2d(self.inplanes)
        self.fusedconv0_2 = fused_conv_bn(conv0_2, bn0_2)
        self.relu = nn.ReLU(inplace=True)

        self.maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.layer1 = self._make_layer(block, self.output_channel_block[0], layers[0])
        conv1 = nn.Conv2d(self.output_channel_block[0], self.output_channel_block[
                               0], kernel_size=3, stride=1, padding=1, bias=False)
        bn1 = nn.BatchNorm2d(self.output_channel_block[0])
        self.fusedconv1 = fused_conv_bn(conv1, bn1)

        self.maxpool2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.layer2 = self._make_layer(block, self.output_channel_block[1], layers[1], stride=1)
        conv2 = nn.Conv2d(self.output_channel_block[1], self.output_channel_block[
                               1], kernel_size=3, stride=1, padding=1, bias=False)
        bn2 = nn.BatchNorm2d(self.output_channel_block[1])
        self.fusedconv2 = fused_conv_bn(conv2, bn2)

        self.maxpool3 = nn.MaxPool2d(kernel_size=2, stride=(2, 1), padding=(0, 1))
        self.layer3 = self._make_layer(block, self.output_channel_block[2], layers[2], stride=1)
        conv3 = nn.Conv2d(self.output_channel_block[2], self.output_channel_block[
                               2], kernel_size=3, stride=1, padding=1, bias=False)
        bn3 = nn.BatchNorm2d(self.output_channel_block[2])
        self.fusedconv3 = fused_conv_bn(conv3, bn3)

        self.layer4 = self._make_layer(block, self.output_channel_block[3], layers[3], stride=1)
        conv4_1 = nn.Conv2d(self.output_channel_block[3], self.output_channel_block[
                                 3], kernel_size=2, stride=(2, 1), padding=(0, 1), bias=False)
        bn4_1 = nn.BatchNorm2d(self.output_channel_block[3])
        self.fusedconv4_1 = fused_conv_bn(conv4_1, bn4_1)
        conv4_2 = nn.Conv2d(self.output_channel_block[3], self.output_channel_block[
                                 3], kernel_size=2, stride=1, padding=0, bias=False)
        bn4_2 = nn.BatchNorm2d(self.output_channel_block[3])
        self.fusedconv4_2 = fused_conv_bn(conv4_2, bn4_2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsample = fused_conv_bn(conv, bn)
    ...
    def forward(self, x):
        x = self.fusedconv0_1(x)
        x = self.relu(x)
        x = self.fusedconv0_2(x)
        x = self.relu(x)
        ...
        x = self.layer4(x)
        x = self.fusedconv4_1(x)
        x = self.relu(x)
        x = self.fusedconv4_2(x)
        x = self.relu(x)
